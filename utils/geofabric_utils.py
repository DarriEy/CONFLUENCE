...
class GeofabricDelineator:
    """
    Handles the delineation of geofabrics using TauDEM.

    This class provides methods for running TauDEM commands, processing DEMs,
    and generating river networks and basins.
    ...
    def delineate_geofabric(self) -> Tuple[Optional[Path], Optional[Path]]:
        """
        Delineate the geofabric for the current domain.
    ...
    def run_taudem_steps(self, dem_path: str, pour_point_path: str):
        """
        Run TauDEM processing steps for geofabric delineation.
    ...
    def run_gdal_processing(self):
        """Run GDAL processing to convert watershed raster to polygon."""
    ...
    def subset_upstream_geofabric(self) -> Tuple[Optional[Path], Optional[Path]]:
        """
        Subset the geofabric to include only upstream basins.

        Returns:
            Tuple[Optional[Path], Optional[Path]]: Paths to the subset river network and river basins shapefiles.
        """
        try:
            basins_path = f"{self.interim_dir}/basin-watersheds.shp"
            rivers_path = f"{self.interim_dir}/basin-streams.shp"
    ...
    def load_geopandas(self, path: Path) -> gpd.GeoDataFrame:
        ...
        Args:
            path (Path): Path to the shapefile.
