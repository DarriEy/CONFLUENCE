...
class DDSOptimizer:
    ...
    def _run_parameter_extraction_summa(self, extract_dir):
        ...
        if summa_path == 'default':
            summa_path = Path(self.config.get('CONFLUENCE_CODE_DIR')) / 'installs' / 'summa' / 'bin'
        else:
        ...
        try:
            ...
            with xr.open_dataset(timestep_file) as ds:
                ...
                for param in available_params:
                    ...
                    if 'hru' in var.dims:
                        # Parameter per HRU - take the first timestep
                        if 'time' in var.dims:
                            param_values[param] = var.isel(time=0).values
                        else:
                            param_values[param] = var.values
                        self.logger.debug(f"Extracted {param} values for {len(param_values[param])} HRUs")
                    elif 'gru' in var.dims:
                        # Parameter per GRU - take the first timestep
                        if 'time' in var.dims:
                            param_values[param] = var.isel(time=0).values
                        else:
                            param_values[param] = var.values
    ...
    def _generate_trial_params_file(self, params):
        ...
        try:
            ...
            with nc.Dataset(self.attr_file_path, 'r') as ds:
                hru_ids = ds.variables['hruId'][:]
                # Try to read hru2gruId if available
                hru2gru_ids = ds.variables['hru2gruId'][:] if 'hru2gruId' in ds.variables else None
                ...
                with nc.Dataset(trial_params_path, 'w', format='NETCDF4') as output_ds:
                    ...
                    for param_name, param_values in params.items():
                        ...
                        if param_name in self.local_params:
                            # Local parameter - directly assign HRU values
                            param_var[:] = param_values
                        else:
                            # Basin parameter - map GRU values to HRUs
                            if hru2gru_ids is not None:
                                for hru_idx in range(len(hru_ids)):
                                    gru_id = hru2gru_ids[hru_idx]
                                    # Find the index of this GRU ID in the gru_ids array
                                    gru_idx = int(gru_id) - 1  # Assuming GRU IDs start from 1
                                    if 0 <= gru_idx < len(param_values):
                                        param_var[hru_idx] = param_values[gru_idx]
                                    else:
                                        param_var[hru_idx] = param_values[0]  # Use first value as fallback
                            else:
                                # No GRU mapping available, use same value for all HRUs
                                param_var[:] = param_values[0]
    ...
    def _run_mizuroute_simulation(self):
        ...
        if mizu_path == 'default':
            mizu_path = Path(self.config.get('CONFLUENCE_CODE_DIR')) / 'installs' / 'mizuRoute' / 'route' / 'bin'
        else:
