...
def setup_confluence_directory(dataset, watershed_id, basin_shapefile_path, basin_filename, basin_id_column=None, is_multibasin=False, geometry=None):
    ...
    try:
        ...
        print("Warning: geopandas not installed. Cannot modify shapefile attributes.")
        return None, False
...
def copy_attributes(dataset, watershed_id, domain_dir):
    ...
    caravan_base = Path("/home/darri.eythorsson/caravan/usr/local/google/home/kratzert/Data/Caravan-csv/Caravan")
...
def generate_config_file(template_path, output_path, domain_name, basin_name, 
    ...
    # Adjust the experiment period based on available data
    # For now, set a generic period of 2000-2010
    config_content = re.sub(r'EXPERIMENT_TIME_START:.*', f'EXPERIMENT_TIME_START: 2000-01-01 00:00', config_content)
    config_content = re.sub(r'EXPERIMENT_TIME_END:.*', f'EXPERIMENT_TIME_END: 2010-12-31 23:00', config_content)
    config_content = re.sub(r'CALIBRATION_PERIOD:.*', f'CALIBRATION_PERIOD: 2000-01-01, 2005-12-31', config_content)
    config_content = re.sub(r'EVALUATION_PERIOD:.*', f'EVALUATION_PERIOD: 2006-01-01, 2010-12-31', config_content)
...
def find_watershed_shapefiles(caravan_dir, dataset):
    ...
    try:
        ...
        print("Error: geopandas is required for shapefile processing. Please install it with pip install geopandas")
        return {}
...
def main():
    ...
    parser.add_argument('--template', type=str, 
                        default='/home/darri.eythorsson/code/CONFLUENCE/0_config_files/config_caravan_template.yaml',
    ...
    for watershed_id in watershed_ids:
        ...
        generate_config_file(
            ...
            config_path, 
            domain_name, 
            target_basin_filename,  # Use the new target filenames for consistency 
            target_river_filename,  # Use the new target filenames for consistency
            bounding_box,
            pour_point,
            streamflow_file
        )
